#!/usr/bin/env python3
"""
SwarmSignal-2B Training Script — Blackwell Edition
=====================================================

bf16 full LoRA on RTX PRO 6000 Blackwell (96GB).
28.5K AI signal writer pairs from Together.ai cook.
Target: GGUF Q4_K_M (~1.5GB) for Jetson Orin Nano (8GB).

Base model:  Qwen/Qwen3.5-2B
Method:      bf16 LoRA r=64 (full precision — 96GB makes 2B trivial)
GPU:         RTX PRO 6000 Blackwell 96GB (CUDA_VISIBLE_DEVICES=1)
Data:        ~28.5K train + 1.5K eval (AI signal writer pairs)
Output:      /data2/swarmsignal-2b/

Run:
    CUDA_DEVICE_ORDER=PCI_BUS_ID CUDA_VISIBLE_DEVICES=1 \
    WANDB_MODE=disabled python3 train_swarmsignal_2b.py
"""
...
MODEL_NAME  = "SwarmSignal-2B-v1"
OUTPUT_DIR  = Path("/data2/swarmsignal-2b")
DATA_DIR    = Path("/data2/swarmsignal-9b/data")       # Same data as 9B
TRAIN_FILE  = DATA_DIR / "swarmsignal_train.jsonl"
EVAL_FILE   = DATA_DIR / "swarmsignal_eval.jsonl"

# bf16 full LoRA — 2B on 96GB is trivial
MAX_SEQ_LENGTH = 4096
LORA_R         = 64
LORA_ALPHA     = 32
LORA_DROPOUT   = 0.0
...
# Training — larger batch since 2B fits easily in 96GB
BATCH_SIZE    = 8               # 2B is tiny — can push batch size
...
def main():
    ...
    print(f"Target:         Jetson Orin Nano 8GB (GGUF Q4_K_M)")
    print(f"{'='*60}\n")
    ...
    model, _proc = FastLanguageModel.from_pretrained(
        ...
        load_in_4bit=False,     # bf16 full — 96GB Blackwell
    )
    ...
    # ── GGUF Export ────────────────────────────────────────────────
    if not args.skip_gguf and not args.skip_merge:
        print("\nExporting GGUF (f16 + Q4_K_M for Jetson Nano)...")
        try:
            merged_dir = OUTPUT_DIR / "merged"
            gguf_f16  = OUTPUT_DIR / "swarmsignal-2b-f16.gguf"
            gguf_q4km = OUTPUT_DIR / "swarmsignal-2b-q4km.gguf"

            # Use llama.cpp for conversion
            import subprocess
            llama_cpp = Path("/home/swarm/llama.cpp")

            if (llama_cpp / "convert_hf_to_gguf.py").exists():
                # Convert to f16 GGUF
                ...
                if quantize_bin.exists():
                    ...
                    print(f"  Q4_K_M GGUF -> {gguf_q4km} ({gguf_q4km.stat().st_size / 1e9:.1f} GB)")
                else:
                    print(f"  WARNING: llama-quantize not found, skipping Q4_K_M")
            else:
                print(f"  WARNING: convert_hf_to_gguf.py not found at {llama_cpp}, skipping GGUF")
        except Exception as e:
            print(f"  GGUF export failed: {e}")
            print("  You can manually convert later with llama.cpp")

    print(f"\n{'='*60}")
    print(f"{MODEL_NAME} — DONE")
    print(f"  Adapter:  {OUTPUT_DIR / 'final'}")
    print(f"  Merged:   {OUTPUT_DIR / 'merged'}")
    print(f"  GGUF:     {OUTPUT_DIR / 'swarmsignal-2b-q4km.gguf'}")
    print(f"  Target:   Jetson Orin Nano 8GB")
